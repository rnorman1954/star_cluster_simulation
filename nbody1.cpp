...
#include  <time.h>						   // for clock_t type and function clock()
using namespace std;
...
int main(int argc, char *argv[])
{
    ...
    get_snapshot(mass, pos, vel, n);            //Read the initial values from stdin
    put_snapshot(mass, pos, vel, n, 0.0);       //Output the initial values to stdout
	...
	double time_tot_s = ((double)(time_end - time_start))/CLOCKS_PER_SEC;  //seconds
	double time_tot_m = time_tot_s/60.0;	//minutes
	cerr << "Execution time of evolve: " << time_tot_s << " seconds";
		cerr << "       = " << time_tot_m << " minutes" << endl;
